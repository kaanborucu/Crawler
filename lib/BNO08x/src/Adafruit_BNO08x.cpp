...
// Keep the original per-transaction wait used by the tested Adafruit driver.
// The initialization state machine provides the larger stage-level deadlines.
static constexpr uint32_t kInitializationWaitTimeoutUs = 500000;
static uint32_t _wait_timeout_us = kInitializationWaitTimeoutUs;
static bool _reset_on_wait_timeout = true;
...
bool Adafruit_BNO08x::begin_SPI(uint8_t cs_pin, uint8_t int_pin,
  ...
  i2c_dev = NULL;
  _event_driven = false;
  _wait_timeout_us = kInitializationWaitTimeoutUs;
  _reset_on_wait_timeout = true;
  ...
  spi_dev = new Adafruit_SPIDevice(cs_pin,
                                   3000000,               // frequency (BNO08x maximum)
...
bool Adafruit_BNO08x::reinitialize(int32_t sensor_id) {
  _event_driven = false;
  _wait_timeout_us = kInitializationWaitTimeoutUs;
  _reset_on_wait_timeout = true;

  // Reuse the same reset, startup/advertisement, product-ID, and callback
  // setup sequence as begin_SPI without creating a second HAL instance.
  sh2_close();
  return _init(sensor_id);
}
...
void Adafruit_BNO08x::setEventDriven(bool enabled, uint32_t waitTimeoutUs,
                                     bool resetOnTimeout) {
  _event_driven = enabled;
  _wait_timeout_us = enabled ? 0 : waitTimeoutUs;
  _reset_on_wait_timeout = !enabled && resetOnTimeout;
}
...
static bool spihal_wait_for_int(void) {
  if (_event_driven && digitalRead(_int_pin)) {
    return false;
  }
  if (!_event_driven && _wait_timeout_us == 0) {
    return true;
  }
  ...
  for (;;) {
    ...
    yield();
    if (_wait_timeout_us == 0 || micros() - startUs >= _wait_timeout_us) {
      break;
    }
  }
  if (_reset_on_wait_timeout) hal_hardwareReset();
...
static int spihal_read(sh2_Hal_t *self, uint8_t *pBuffer, unsigned len,
  ...
  // SH-2 uses this timestamp to reconstruct the report event time. Record
  // the instant the BNO08x interrupt is observed instead of leaving the HAL
  // output parameter at zero.
  *t_us = micros();
