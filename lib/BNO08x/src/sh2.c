...
#if defined(ARDUINO)
#include <Arduino.h>
#define SH2_YIELD() yield()
#else
#define SH2_YIELD() ((void)0)
#endif

#define SH2_DEFAULT_OPERATION_TIMEOUT_US (1000000U)
...
#define ADVERT_TIMEOUT_US (3000000U)
...
static int opProcess(sh2_t *pSh2, const sh2_Op_t *pOp)
{
    int status = SH2_OK;
    uint32_t start_us = 0;
    const uint32_t operationTimeoutUs =
        pOp->timeout_us == 0 ? SH2_DEFAULT_OPERATION_TIMEOUT_US
                             : pOp->timeout_us;
    ...
    while ((pSh2->pOp != 0) &&
           ((now_us-start_us) < operationTimeoutUs)) {
