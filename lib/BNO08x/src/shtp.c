...
#if defined(ARDUINO)
#include <Arduino.h>
#define SHTP_YIELD() yield()
#else
#define SHTP_YIELD() ((void)0)
#endif
...
#define SHTP_TX_RETRY_BUDGET_US (20000U)
#define SHTP_TX_MAX_WRITE_RETRIES (16U)
#define SHTP_ASSEMBLY_TIMEOUT_US (10000U)
...
static int txProcess(shtp_t *pShtp, uint8_t chan, const uint8_t* pData, uint32_t len)
{
    ...
    while (remaining > 0) {
        ...
        const uint32_t retryStartUs =
            pShtp->pHal->getTimeUs(pShtp->pHal);
        uint32_t retryCount = 0;
        while (status == 0 && retryCount < SHTP_TX_MAX_WRITE_RETRIES &&
               (pShtp->pHal->getTimeUs(pShtp->pHal) - retryStartUs) <
                   SHTP_TX_RETRY_BUDGET_US) {
            shtp_service(pShtp);
            SHTP_YIELD();
            ++retryCount;
            status = pShtp->pHal->write(pShtp->pHal, pShtp->outTransfer, lenField);
...
static void rxAssemble(shtp_t *pShtp, uint8_t *in, uint16_t len, uint32_t t_us)
{
    ...
    // Discard an incomplete assembly that has been waiting too long.
    if (pShtp->inRemaining &&
        (t_us - pShtp->inAssemblyStartUs) >= SHTP_ASSEMBLY_TIMEOUT_US) {
        pShtp->inRemaining = 0;
        pShtp->inCursor = 0;
        pShtp->inAssemblyStartUs = 0;
    }
