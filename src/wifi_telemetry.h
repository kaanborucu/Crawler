...
struct TelemetrySnapshot {
  ...
  float imuAccelRequestedHz;
  float imuAccelMeasuredHz;
  uint32_t imuAccelAgeUs;
  bool imuAccelValid;
  uint8_t imuAccelAccuracy;
  uint32_t imuAccelSequenceGaps;
  uint32_t imuAccelTimestampBacksteps;
  float imuGyroRequestedHz;
  float imuGyroMeasuredHz;
  uint32_t imuGyroAgeUs;
  bool imuGyroValid;
  uint8_t imuGyroAccuracy;
  uint32_t imuGyroSequenceGaps;
  uint32_t imuGyroTimestampBacksteps;
  bool imuResetRecoveryActive;
  uint32_t imuResetCount;
  uint32_t imuResetGeneration;
  uint32_t imuLastDrainUs;
  uint32_t imuMaxDrainUs;
  uint32_t imuLastEventsPerDrain;
  uint32_t imuMaxEventsPerDrain;
  uint32_t imuDrainBudgetHits;
  uint32_t imuEventLimitHits;
  uint32_t imuTransactionFailures;
  uint32_t imuRecoveryAttempts;
  uint32_t imuRecoveryFailures;
  uint8_t imuLastFailureStage;
